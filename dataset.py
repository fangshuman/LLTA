...
class ImageNetDataset(torch.utils.data.Dataset):
    def __init__(self, image_dir, label_path, total=None, size=224):
        self.image_dir = image_dir
        self.class_to_idx = np.load(label_path, allow_pickle=True)[()]
...
def make_loader(image_dir, label_path, batch_size=1, total=None, size=224):
    imgset = ImageNetDataset(image_dir, label_path, total=total, size=size)
